...
{
    // From http://msdn.microsoft.com/en-us/magazine/cc163823.aspx
    ...
    internal
#endif
        delegate void PrivilegedCallback(object state);
        ...
        sealed class Privilege
    {
        ...
        private sealed class TlsContents : IDisposable
        {
            ...
            private SafeTokenHandle threadHandle = new SafeTokenHandle();
            private bool isImpersonating = false;

            private static SafeTokenHandle processHandle = new SafeTokenHandle();
            ...
            public TlsContents()
            {
                ...
                if (processHandle.IsInvalid)
                {
                    lock (syncRoot)
                    {
                        if (processHandle.IsInvalid)
                        {
                            if (OpenProcessToken(GetCurrentProcess(), TokenAccessLevels.Duplicate, ref processHandle))
                            {
            ...
            private void Dispose(bool disposing)
            {
                ...
                if (this.threadHandle != null)
                {
                    ...
                    this.threadHandle = new SafeTokenHandle();
                }
        ...
        {
            var privMods = privs.Select(p => new Privilege(p)).ToArray();
            ...
            finally
            {
                ...
                finally
                {
                    ...
                    if (exception != null)
                    {
                        throw new InvalidOperationException("Could not revert all privileges", exception);
                    }
                }
            }
        }
